...
function load_data(file_name) {
  d3.csv(file_name).then(function(sources) {
    svg.selectAll(".foreground path").remove();
    data = sources;
    // if (error) throw error;
    var values = sources.flatMap(d => 
      Object.keys(d).filter(k => k !== "animal" && k !== "total").map(k => +d[k])
      // d3.keys(d).filter(k => k !== "animal" && k !== "total").map(k => +d[k])
    );
    // var uniform_scale = d3.scale.linear()
    // .domain(d3.extent(values))
    // .range([height, 0]);
    var uniform_scale = d3.scaleLinear()
    .domain(d3.extent(values))
    .range([height, 0]);

    //Extract the list of dimensions and create a scale for each.
    x.domain(dimensions = Object.keys(sources[0]).filter(function(d) {
