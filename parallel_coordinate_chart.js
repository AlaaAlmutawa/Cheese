...
function filter_lines(category) {
  ...
  if(moving_icons){ //moving icons functionality is only possible if the user selected to display the mean values 
    ...
    var selected_icon = svg.append("svg:image")
        .attr("class", "moving_icon")
        .attr('width',40)
        .attr('height',40)
        .attr('xlink:href',"images/"+category+".png")
...
d3.select("#user_selection").on("change", function() {
  ...
  var file_name = selected_option === "all" ? "data/stages-all-clim_change1.csv" : "data/stages-clim_change1.csv";
...
let file_name = 'data/stages-clim_change1.csv'
load_data(file_name);
